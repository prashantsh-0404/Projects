...
<pre class="prettyprint source lang-bash"><code>npm install gremlin</code></pre><p>Gremlin-Javascript is designed to connect to a &quot;server&quot; that is hosting a TinkerPop-enabled graph system. That &quot;server&quot; 
could be <a href="http://tinkerpop.apache.org/docs/current/reference/#gremlin-server">Gremlin Server</a> or a <a href="http://tinkerpop.apache.org/docs/current/reference/#connecting-rgp">remote Gremlin provider</a> that exposes protocols by which Gremlin-Javascript 
can connect.</p>
...
const g = traversal().withRemote(new DriverRemoteConnection('ws://localhost:8182/gremlin'));</code></pre><p>Once &quot;g&quot; has been created using a connection, it is then possible to start writing Gremlin traversals to query the 
remote graph:</p>
<pre class="prettyprint source lang-javascript"><code>g.V().hasLabel('person').values('name').toList()
  .then(names => console.log(names));

const names = await g.V().hasLabel('person').values('name').toList();
console.log(names);</code></pre><p>Please see the <a href="http://tinkerpop.apache.org/docs/current/reference/#gremlin-javascript">reference documentation</a> at Apache TinkerPop for more information.</p>
